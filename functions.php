...
function addRepresentativeTable() {
    global $pdo;
...
function addRepresentative($user_id, $discount_price) {
    global $pdo;
    $stmt = $pdo->prepare("INSERT INTO representatives (user_id, discount_price) VALUES (?, ?)");
    $stmt->execute([$user_id, $discount_price]);
}

function setDiscountPrice($user_id, $discount_price) {
    global $pdo;
    $stmt = $pdo->prepare("UPDATE representatives SET discount_price = ? WHERE user_id = ?");
    $stmt->execute([$discount_price, $user_id]);
}

function incrementSales($user_id) {
    global $pdo;
    $stmt = $pdo->prepare("UPDATE representatives SET total_sales = total_sales + 1 WHERE user_id = ?");
...
function getSalesReport() {
    global $pdo;
    $stmt = $pdo->query("SELECT u.username, r.total_sales FROM representatives r JOIN user u ON r.user_id = u.id");
