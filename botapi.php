...
function telegram($method, $datas = [])
{
    ...
    if (!$res['ok']) {
        error_log(json_encode($res));  // لاگ خطا در صورت وجود
    }
    if (curl_error($ch)) {
        error_log(curl_error($ch));  // در صورت بروز خطا در CURL
    } else {
        return $res;
    }
}
...
// بررسی ورودی‌های مختلف پیام‌های تلگرام
...
#در این قسمت ورودی‌ها به درستی خوانده شده‌اند
// اضافه کردن بررسی و پردازش داده‌ها اگر نیاز باشد
?>
