...
if (preg_match('/reject_pay_(\w+)/', $datain, $datagetr)) {
    ...
    if ($Payment_report['payment_Status'] == "reject" || $Payment_report['payment_Status'] == "paid") {
        ...
        // پردازش درخواست‌ها از دکمه‌ها
if ($callback_query_data == 'add_representative') {
    // نمایش فرم افزودن نماینده
    sendmessage($from_id, "لطفاً شناسه کاربری و قیمت ویژه نماینده را وارد کنید.");
} elseif ($callback_query_data == 'view_sales_report') {
    // نمایش گزارش فروش‌ها
    $salesReport = getSalesReport(); // این تابع را در `functions.php` داشتیم
    $reportText = "گزارش فروش‌ها:\n";
    foreach ($salesReport as $report) {
        $reportText .= "نام نماینده: " . $report['username'] . " - فروش‌ها: " . $report['total_sales'] . "\n";
    }
    sendmessage($from_id, $reportText, $keyboard, 'HTML');
}

// دریافت اطلاعات نماینده از کاربر
if ($text) {
    // فرض کنیم که شناسه کاربری و قیمت ویژه به صورت جداگانه ارسال شده‌اند
    $user_id = $text;  // این باید شناسه کاربری باشد که از کاربر دریافت می‌شود
    $discount_price = 10.00;  // به عنوان مثال قیمت ویژه

    addRepresentative($user_id, $discount_price); // تابعی که قبلاً در `functions.php` داشتیم
    sendmessage($from_id, "نماینده جدید با موفقیت افزوده شد.");
}
...
if ($text == $textbotlang['Admin']['keyboardadmin']['user_search']) {
    ...
    'inline_keyboard' => [
        ...
        [['text' => 'مشاهده فروش‌های نمایندگان', 'callback_data' => 'view_sales_report']],
    ]
];
